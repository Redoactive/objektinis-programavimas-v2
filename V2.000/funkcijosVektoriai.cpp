...
static bool arSkirtingiVektoriai = false;
    studentai_class a;
TEST(puikusTestas, testuojamaArKartojas) {
    EXPECT_NE(a.getVardas(), a.getPavarde()) << "Vardas ir pavarde yra ta pati";
}
TEST(puikusTestas, testuojamaArVidurkisGeras) {
    EXPECT_GE(a.getVidurkis(), 5) << "Vidurkis turi buti ne mazesnis negu 5!";
}
TEST(puikusTestas, testuojamaArMedianaGeras) {
    EXPECT_GE(a.getMediana(), 5) << "Mediana turi buti ne mazesnis negu 5!";
}
TEST(puikusTestas, testuojamaPazymiuKiekis) {
    EXPECT_GE(a.getBalai().size(), 3) << "Balu turi buti ne maziau 3";
}
